...
function baseStyles() {
  return `
  :root {
    --bg: #0b1220; --panel: #131c2e; --panel-2: #1a2538; --border: #243047;
    --text: #e8eef7; --muted: #8b9bb4; --accent: #3b9eff;
  ...
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  input,textarea,select{width:100%;padding:11px 14px;border-radius:8px;border:1px solid var(--border);background:var(--bg);color:var(--text);font-size:.95rem;font-family:inherit;margin-bottom:16px}
  ...
  .alert-ok{background:var(--ok-bg);color:var(--ok);border:1px solid rgba(34,197,94,.3)}
  .alert-info{background:var(--accent-soft);color:var(--accent);border:1px solid rgba(59,158,255,.3)}
  .filters{display:flex;gap:8px;flex-wrap:wrap;margin-bottom:16px;align-items:center}
  .filter-btn{padding:6px 12px;border-radius:20px;font-size:.8rem;text-decoration:none;color:var(--muted);background:var(--panel-2);border:1px solid transparent}
  ...
  .qr-box{text-align:center;margin:20px 0;padding:16px;background:var(--bg);border-radius:10px;border:1px solid var(--border)}
  .qr-box img{width:160px;height:160px;border-radius:8px;background:#fff;padding:8px}
  .history{margin-top:24px;border-top:1px solid var(--border);padding-top:16px}
  .history h3{font-size:.9rem;color:var(--muted);margin-bottom:10px;text-transform:uppercase;letter-spacing:.04em}
  .history li{font-size:.8rem;color:var(--muted);padding:4px 0;list-style:none}
  @media(max-width:768px){
    ...
    .sidebar-body{display:none;padding:8px 14px 16px;border-top:1px solid var(--border)}
...
function adminHomePage(stats, origin) {
  ...
  return `<!DOCTYPE html>
<html lang="es"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Inicio – Breto's Services</title><style>${baseStyles()}</style></head><body>
${sidebar("inicio")}
<main class="main">
  ...
  <div class="card">
    <p style="color:var(--muted);line-height:1.6">Gestiona tus tarjetas NFC/QR de reseñas Google desde Dispositivos.</p>
...
function adminListPage(filtered, all, filter, q, msg, origin) {
  ...
  const rows = filtered.length === 0
    ...
    : filtered.map(d => {
      ...
      <tr>
        ...
        <td>
          <button type="button" class="btn btn-ghost btn-sm" onclick="copyLink('${shortUrl}', this)">Copiar enlace</button>
        </td>
        <td>
          <a href="/admin/edit/${d.code}" class="btn btn-primary btn-sm">Configurar</a>
        </td>
      </tr>`;
      }).join('');
  ...
  return `<!DOCTYPE html>
<html lang="es"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Dispositivos – Breto's Services</title><style>${baseStyles()}</style></head><body>
${sidebar("dispositivos")}
<main class="main">
  <div class="page-header" style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:12px">
    <div><h1>Dispositivos</h1><p>${filtered.length} de ${all.length}</p></div>
    <div class="actions">
      <a href="/admin/new" class="btn btn-primary">+ Nuevo</a>
      <a href="/admin/export" class="btn btn-ghost">Exportar CSV</a>
    </div>
  </div>
  ${flashMsg(msg)}
...
function editFormPage(code, data, origin, msg) {
  ...
  return `<!DOCTYPE html>
<html lang="es"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Configurar ${code}</title><style>${baseStyles()}</style></head><body>
${sidebar("dispositivos")}
<main class="main">
  ...
  <div class="card" style="max-width:520px">
    <form method="POST" action="/admin/edit/${code}">
      ...
      <div class="actions" style="margin-bottom:16px">
        <button type="button" class="btn btn-ghost btn-sm" onclick="
          var n = document.getElementById('businessName').value.trim();
          if(n) window.open('https://www.google.com/maps/search/' + encodeURIComponent(n), '_blank');
          else alert('Escribe primero el nombre del negocio');
        ">Buscar en Google Maps</button>
      </div>
    ...
    <div class="qr-box">
      <p style="font-size:.8rem;color:var(--muted);margin-bottom:10px">QR del enlace corto</p>
      <img src="${qrUrl}" alt="QR ${code}">
      <p style="font-size:.8rem;margin-top:10px;word-break:break-all;color:var(--accent)">${shortUrl}</p>
      <button type="button" class="btn btn-ghost btn-sm" style="margin-top:8px" onclick="copyLink('${shortUrl}', this)">Copiar enlace</button>
    </div>

    <form method="POST" action="/admin/delete/${code}" onsubmit="return confirm('¿Eliminar ${code}? Esta acción no se puede deshacer.')">
      <button type="submit" class="btn btn-danger btn-block">Eliminar dispositivo</button>
    </form>
    ...
    <div class="history">
      ...
      <ul>
        ${history.slice(0, 8).map(h => `
          <li>${h.action} · ${h.businessName || ''} · ${h.at ? new Date(h.at).toLocaleString('es-CL') : ''}</li>
        `).join('')}
      </ul>
    </div>` : ''}
  </div>
...
function newDevicePage() {
  return `<!DOCTYPE html>
<html lang="es"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Nuevo dispositivo – Breto's Services</title><style>${baseStyles()}</style></head><body>
${sidebar("dispositivos")}
<main class="main">
...
function domainPage(origin) {
  return `<!DOCTYPE html>
<html lang="es"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Dominio – Breto's Services</title><style>${baseStyles()}</style></head><body>
${sidebar("dominio")}
<main class="main">
  ...
  <div class="card" style="max-width:560px">
    <p style="color:var(--muted);line-height:1.6;margin-bottom:16px">
      Actualmente usas: <strong style="color:var(--accent)">${origin}</strong>
    </p>
    <p style="color:var(--muted);line-height:1.6;margin-bottom:16px">
      Para usar un dominio propio (ej: <code>go.bretosservices.cl</code>):
    </p>
    <ol style="color:var(--muted);line-height:1.8;padding-left:20px;margin-bottom:20px">
      ...
      <li>Configura el registro DNS (CNAME o A) que te indique Cloudflare</li>
    </ol>
    <p style="color:var(--muted);font-size:.85rem">Esto no requiere cambios de código. El panel y los redirects seguirán funcionando igual.</p>
  </div>
</main></body></html>`;
}
...
function placeholderPage(section) {
  ...
  return `<!DOCTYPE html>
<html lang="es"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>${title} – Breto's Services</title><style>${baseStyles()}</style></head><body>
${sidebar(section)}
<main class="main">
  ...
  <div class="card empty">
    ...
    <p style="margin-top:12px;font-size:.85rem">Base lista para conectar CRM, pedidos, packs y tarifas.</p>
  </div>
</main></body></html>`;
}
